...
export const runApiTests = async () => {
  ...
  try {
    ...
    // 2. 发布宠物 (Create Pet)
    log('2. 测试发布宠物接口...');
    const petRes = await apiPost('/api/pets', {
    ...
    if (!petId) throw new Error('未返回宠物ID');
    log(`发布宠物成功 (ID: ${petId})`, 'success');

    // 3. 获取宠物列表 (Get Pets)
    log('3. 测试获取宠物列表接口...');
    const petsRes = await apiGet('/api/pets');
    if (!Array.isArray(petsRes.data)) throw new Error('返回格式错误');
    log(`获取宠物列表成功 (当前数量: ${petsRes.data.length})`, 'success');

    // 4. 提交救助记录 (Create Rescue Record)
    log('4. 测试提交救助记录接口...');
    await apiPost('/api/rescue-records', {
      petId: petId,
      rescuerName: '[测试] 救助侠B',
    ...
    log('5. 测试获取救助历史接口...');
    const historyRes = await apiGet(`/api/rescue-records/pet/${petId}`);
    if (!historyRes.data || historyRes.data.length === 0) throw new Error('未找到刚才提交的记录');
    ...
    log('7. 测试发布日常接口...');
    const postRes = await apiPost('/api/community/posts', {
      petId: petId,
    ...
    if (!postId) throw new Error('未返回帖子ID');
    log(`发布日常成功 (ID: ${postId})`, 'success');

    // 8. 获取日常列表 (Get Posts)
    log('8. 测试获取日常列表接口...');
    await apiGet(`/api/community/posts?petId=${petId}`);
    log('获取日常列表成功', 'success');

    // 9. 获取单条日常 (Get Single Post)
    log('9. 测试获取单条日常接口...');
    await apiGet(`/api/community/posts/${postId}`);
    log('获取单条日常成功', 'success');

    // 10. 发表评论 (Create Comment)
    log('10. 测试发表评论接口...');
    await apiPost(`/api/community/posts/${postId}/comments`, {
      content: '太棒了！',
      authorName: '围观群众C',
    ...
    log('11. 测试获取评论列表接口...');
    const commentsRes = await apiGet(`/api/community/posts/${postId}/comments`);
    if (!commentsRes.data || commentsRes.data.length === 0) throw new Error('未找到刚才发表的评论');
    log('获取评论列表成功', 'success');

    // 12. 通知相关 (Notifications)
    log('12. 测试通知相关接口...');
    // 检查救助侠B是否收到评论通知
    const notifRes = await apiGet('/api/community/notifications?openId=rescuer-001');
    log(`获取通知列表成功 (数量: ${notifRes.data.length})`, 'success');
    
    await apiGet('/api/community/notifications/unread-count?openId=rescuer-001');
    log('获取未读数成功', 'success');
    
    await apiPost('/api/community/notifications/read', { openId: 'rescuer-001' });
    log('标记已读成功', 'success');

    // 13. 统计数据 (Statistics)
    log('13. 测试统计数据接口...');
    await apiGet('/api/statistics/overview');
    await apiGet('/api/statistics/trends');
    await apiGet('/api/statistics/regions');
