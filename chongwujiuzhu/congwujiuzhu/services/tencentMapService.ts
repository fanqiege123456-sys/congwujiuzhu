 ...
 * @returns 返回搜索结果数组
 */
export async function searchPlace(keyword: string, region?: string): Promise<any[]> {
  return new Promise((resolve, reject) => {
    wx.request({
 ...
 * @returns 返回距离（单位：米）
 */
export function calculateDistanceWithMap(
  lat1: number,
  lng1: number,
  lat2: number,
  lng2: number
 ...
 * @param title 位置标题
 */
export function openMapNavigation(latitude: number, longitude: number, title: string = '目标位置'): void {
  // 在微信小程序中打开地图导航
  wx.openLocation({
    latitude: latitude,
    longitude: longitude,
